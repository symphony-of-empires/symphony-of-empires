...
#include <stdio.h>
static __always_inline void Province_DoTick(World * world, Province * province) {
	...
	for(size_t i = 0; i < province->n_products; i++) {
		...
		//printf("product (based %s) now cost %f (%zu, %zu)\n", world->goods[product->good_id].ref_name, product->price, product->supply, product->demand);
	}
	//printf("budget: %f\n", province->budget);
}
...
void World_DoTick(World * world) {
	for(size_t i = 0; i < world->n_provinces; i++) {
		...
		Province_DoTick(world, province);
	}
	world->time++;
}
...
void World_Create(World * world, const char * topo_map, const char * pol_map, const char * div_map) {
	...
	if(topo.width != world->width || topo.height != world->height
	|| pol.width != world->width || pol.height != world->height
	|| div.width != world->width || div.height != world->height) {
	...
	for(size_t i = 0; i < world->width * world->height; i++) {
		world->tiles[i].elevation = topo.buffer[i] & 0xff;
		
		/* associate tiles with nations */
		world->tiles[i].owner_id = (size_t)-1;
		for(size_t j = 0; j < world->n_nations; j++) {
			if(pol.buffer[i] == world->nations[j].color) {
				world->tiles[i].owner_id = j;
				break;
			}
		}

		/* associate tiles with provinces */
		world->tiles[i].province_id = (size_t)-1;
		for(size_t j = 0; j < world->n_provinces; j++) {
			if(div.buffer[i] == world->provinces[j].color) {
				world->tiles[i].province_id = j;
				break;
			}
		}
	}
