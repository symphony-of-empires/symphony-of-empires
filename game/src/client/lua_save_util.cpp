...
void LUA_util::save(GameState& gs, const std::string_view savefile_path) {
    if(gs.editor) {
        ...
        fprintf(fp.get(), "local v = {}\n");
        for(const auto& nation : gs.world->nations) {
            const uint32_t color = std::byteswap<std::uint32_t>((nation.color & 0x00ffffff) << 8);
            fprintf(fp.get(), "v = Nation:new{ref_name=\"%s\",name=\"%s\",color=\"%s\"}\n", nation.ref_name.data(), nation.name.data(), color);
            const auto& ideology = gs.world->ideologies[nation.ideology_id];
            const auto& subideology = 
            fprintf(fp.get(), "v:set_ideology(Ideology:get(\"%s\"),\"%s\")\n", ideology.ref_name.data(), ideology.subideologies[nation.subideology_id].ref_name.data());
            for(const auto& language : gs.world->languages)
                if(nation.language_acceptance[language] > 0.f)
                    fprintf(fp.get(), "v:add_accepted_language(Language:get(\"%s\"))\n", language.ref_name.data());
            for(const auto& religion : gs.world->religions)
                if(nation.religion_acceptance[religion] > 0.f)
                    fprintf(fp.get(), "v:add_accepted_religion(Religion:get(\"%s\"))\n", religion.ref_name.data());
            cnt++;
        }
        fp.reset();
