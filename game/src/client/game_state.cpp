    ...
    if(gs.editor) {
        ...
        for(auto& province : gs.world->provinces) {
            ...
            for(const auto& pop : province.pops)
                fprintf(fp.get(), "province:add_pop(pt_%s,c_%s,r_%s,%f,%f)\n", gs.world->pop_types[pop.type_id].ref_name.c_str(), gs.world->languages[pop.language_id].ref_name.c_str(), gs.world->religions[pop.religion_id].ref_name.c_str(), pop.size, pop.literacy);
