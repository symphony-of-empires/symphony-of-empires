    ...
    if(!(rand() % 5)) {
        ...
        for(const auto province_id : nation.owned_provinces) {
            ...
            for(const auto& good : world.goods) {
                const auto& product = province.products[good];
                avg_prob[good] += product.demand / (product.supply + 1) * product.price;
            }
        }
        ...
        for(const auto& building_type : world.building_types) // Take in account all buildings for this
            for(const auto& input_id : building_type.input_ids) // Apply the higher-probability with outputs of this factory
                avg_prob[building_type.output_id] += avg_prob[input_id] + 1;
        ...
        for(const auto& building_type : world.building_types) {
            ...
            if(!building_type.input_ids.empty() || Good::is_invalid(building_type.output_id)) continue;
            // Randomness
            if(rand() % 5) continue;
            return &world.goods[building_type.output_id];
        }
    }
...
static inline void ai_build_commercial(Nation& nation) {
    ...
    for(auto& building_type : world.building_types) {
        if(building_type.output_id == *target_good) {
            type = &building_type;
            break;
        }
    }
    ...
    if(nation.ai_do_cmd_troops) {
        ...
        for(const auto& other : world.nations) {
            ...
            if(!relation.is_allied()) // Makes sure so 200 relations results on 0.0 attraction, while 1 or 0 relations result on 3.9 and 4.0
                ai_data.nations_risk_factor[other] = ((relation_range - (relation.relation + relation_max)) / relation_max) * nation_weight;
            if(relation.has_war)
                ai_data.nations_risk_factor[other] = war_weight * nation_weight;
        }
        ai_data.nations_risk_factor[nation] = homeland_weight * nation_weight;
    ...
    if(!(world.time % world.ticks_per_month) && nation.ai_controlled) {
        ...
        if(0) {
            ...
            for(auto& province : colonial_value) {
                ...
                province.second += province.first->total_pops() * 0.005f;
                // Bordering a province of ours means we are **to** colonize it
                for(const auto neighbour_id : province.first->neighbour_ids) {
                    auto& neighbour = g_world.provinces[neighbour_id];
                    if(neighbour.controller_id == nation)
        ...
        if(rand() % (base_reluctance / defense_factor) == 0) {
            ...
            for(const auto province_id : nation.controlled_provinces) {
                if(rand() % (base_reluctance / defense_factor)) continue;
                ...
                for(size_t i = 0; i < world.building_types.size(); i++) {
                    ...
                    if(UnitType::is_valid(building.working_unit_type_id) || !building.can_do_output())
                        continue;
